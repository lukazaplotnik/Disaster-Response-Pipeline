...
def load_data(messages_filepath, categories_filepath):
    """ Load messages and corresponding categorizations, and merge them into a dataframe based on the message ID.
...
def clean_data(df):
    """ Clean the dataframe by splitting the categories column into separate, clearly named columns, converting values     to binary, and dropping duplicates.
    ...
    #Note: There are some instances of duplicate IDs that do not contain the same content in the 'categories' column.       #Ideally, one would manually select which row to keep and which to delete based on the content in a given message.     #However, the aim is to develop an automated ETL approach, therefore we rely on a simpler approach and directly         #use the drop_duplicates method, keeping only the first instance of each duplicate ID. The simplification does not     #lead to significant loss of information, since the number of problematic rows is low.
